...
def get_model_data(dir_path, model_file_name):
    # maybe_download_and_extract(dir_path, model_file_name)
...
def maybe_download_and_extract(dir_path, model_url_from_get_model_data, is_tarfile=False, is_zipfile=False):
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    filename = model_url_from_get_model_data.split('/')[-1]
    filepath = os.path.join(dir_path, filename)
    if not os.path.exists(filepath):
        ...
        filepath, _ = urllib.request.urlretrieve(model_url_from_get_model_data, filepath, reporthook=_progress)
        print()
...
def save_image(image, save_dir, name, mean=None):
    ...
    imsave(os.path.join(save_dir, name + ".png"), image)
