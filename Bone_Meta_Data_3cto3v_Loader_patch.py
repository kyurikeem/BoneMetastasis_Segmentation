...
#---------------------------------------------------------------------------#
# 2018.10.21                               image loader training
#---------------------------------------------------------------------------#
"""
import numpy as np
...
class data_loader:
    ...
    def file_type_check_and_load(self, one_of_list_values, index):
        ...
        if one_of_list_values.split('.')[-1] in ['jpg', 'png']:
            ...
            self.np_array_values = self.mat_file_in_dict[self.specific_key]
    ...
    def get_next_batch(self, batch_size):
        ...
        for i, file_list in enumerate(self.batch_file_list):
            ...
            if 'main_annotIndex' in main_file_dict:
                ...
                # self.pre_img_temp = self.pre_img_temp - (np.mean(self.pre_img_temp))
                # self.main_img_temp = self.main_img_temp - (np.mean(self.main_img_temp))
                # self.post_img_temp = self.post_img_temp - (np.mean(self.post_img_temp))
                ...
                self.pre_img_temp = np.array(self.file_type_check_and_load_negative(pre_file_dict['pre_image']))
                self.main_img_temp = np.array(self.file_type_check_and_load_negative(main_file_dict['main_image']))
                self.post_img_temp = np.array(self.file_type_check_and_load_negative(post_file_dict['post_image']))
