...
def vgg_net(image, layer_set, debuging_option):
    ...
    for i, name in enumerate(name_of_layer):
        ...
        if layer_kind == 'conv':
            ...
            original_kernel = utils.get_variable(np.transpose(kernels, (1, 0, 2, 3)), name='pre_model_' +name + "_w")
            bias = utils.get_variable(bias.reshape(-1), name='pre_model_' + name + "_b") #reshape(-1)은 flatten 역할
            ...
            current_data = tf.nn.relu(current_data, name='pre_model_' + name)

            if debuging_option:
                utils.add_activation_summary(current_data)

        elif layer_kind == 'pool':
            current_data = utils.avg_pool_2x2(current_data, name='pre_model_' + name)
