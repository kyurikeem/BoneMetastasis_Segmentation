...
def vgg_net(image, layer_set, debuging_option):
    ...
    for i, name in enumerate(name_of_layer):
        ...
        if layer_kind == 'conv':
            ...
            original_kernel = utils.get_variable(np.transpose(kernels, (1, 0, 2, 3)), name='main_model_' + name + "_w")
            bias = utils.get_variable(bias.reshape(-1), name='main_model_' + name + "_b") #reshape(-1)은 flatten 역할
            ...
            # [Optional]해당 부분에서 kernel channel 수정
            ...
            current_data = tf.nn.relu(current_data, name='main_model_'+ name)

            if debuging_option:
                utils.add_activation_summary(current_data) #분석 요함

        elif layer_kind == 'pool':
            current_data = utils.avg_pool_2x2(current_data, name='main_model_' + name)
